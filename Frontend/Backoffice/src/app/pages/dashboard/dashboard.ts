...
export class DashboardComponent implements OnInit {
  ...
  topCampsites = signal<any[]>([]);
  searchResults = signal<any[]>([]);
  searchMinCapacity = 10;
  searchRole = 'CAMPSITE_OWNER';
  ...
  ngOnInit() {
    ...
    if (this.isAdmin()) {
      this.campsiteService.getTopByRevenue().subscribe((res: any) => {
        this.topCampsites.set(res?.data || []);
      });
    }
  }

  searchCampsites() {
    this.campsiteService.searchByOwnerAndCapacity(this.searchRole, this.searchMinCapacity)
      .subscribe((res: any) => {
