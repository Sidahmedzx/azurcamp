...
import com.camp.backend.dto.CampsiteRevenuResponse;
...
public class CampsiteController {
    ...
    public ResponseEntity<ApiResponse<List<CampsiteRevenuResponse>>> getTopByRevenue() {
        return ResponseEntity.ok(ApiResponse.ok("Top campsites by revenue", campsiteService.getTopCampsitesByRevenue()));
    }

    @GetMapping("/search")
    public ResponseEntity<ApiResponse<List<CampsiteResponse>>> searchByOwnerAndCapacity(
            @RequestParam UserRole ownerRole,
            @RequestParam Integer minCapacity,
            @RequestParam(defaultValue = "APPROVED") ApprovalStatus status) {
        return ResponseEntity.ok(ApiResponse.ok("Search results",
                campsiteService.findByOwnerRoleAndMinCapacity(ownerRole, minCapacity, status)));
    }
