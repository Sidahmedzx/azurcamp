...
export class CampsiteService {
  ...
  private apiUrl = 'https://camp-backend-hmgdcvhthwdmf2dw.austriaeast-01.azurewebsites.net/api/campsites';
  ...
  getTopByRevenue() {
    return this.http.get<any>(`${this.apiUrl}/top-revenue`);
  }

  searchByOwnerAndCapacity(ownerRole: string, minCapacity: number, status: string = 'APPROVED') {
    return this.http.get<any>(`${this.apiUrl}/search`, {
      params: { ownerRole, minCapacity: minCapacity.toString(), status }
    });
  }
}