...
public class CampsiteService {
    ...
    public List<CampsiteRevenuResponse> getTopCampsitesByRevenue() {
        List<Object[]> results = campsiteRepository.findTopCampsitesByRevenue();
        return results.stream().map(row -> new CampsiteRevenuResponse(
    ...
    public List<CampsiteResponse> findByOwnerRoleAndMinCapacity(UserRole role, Integer minCapacity, ApprovalStatus status) {
        return campsiteRepository
                .findByOwner_RoleAndCapacityGreaterThanEqualAndApprovalStatusOrderByNightlyPriceAsc(role, minCapacity, status)
                .stream()
                .map(this::toResponse)
                .toList();
    }
